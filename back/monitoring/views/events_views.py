...
def get_end_of_contract_events(filters):
...
def get_end_of_warranty_events(filters):
    if filter := filters.get("project"):
        # These events aren't applied for projects
        return []
    with connection.cursor() as cursor:
...
def get_coming_events(request, format=None):
    ...
    events = []
    events += get_end_of_warranty_events(filter)
    events += get_end_of_contract_events(filter)
